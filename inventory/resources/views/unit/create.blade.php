...
@section('content')
    <div class="w-full p-8 mt-5 bg-white rounded-lg shadow-md">
        <h2 class="text-2xl font-semibold mb-6">Thêm đơn vị</h2>
        <form action="{{ route('units.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input name="short_form" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="short_form" type="text" placeholder="Short form">
                @error('short_form')
                    <p class="text-red-500 text-md">{{ $message }}</p>
                @enderror
            </div>
