...
@section('content')
    <div class="w-full p-8 mt-5 bg-white rounded-lg shadow-md">
        <h1 class="text-xl font-semibold mb-6">Chỉnh sửa mua hàng</h1>
        <form action="{{ route('purchases.update', $purchase->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" name="purchase_no" value="{{ $purchase->purchase_no }}" id="purchaseNo" placeholder="Purchase No" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    @error('purchase_no')
                ...
                <div>
                    ...
                    <input type="text" name="paid_amount" value="{{ $purchase->paid_amount }}" id="paidAmount" placeholder="Paid Amount" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    @error('paid_amount')
                        <p class="text-red-500 text-md">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full bg-white">
                    ...
                    <tbody class="tbody">
                        @foreach ($purchase->purchases_meta as $item)
                            <tr class="tr">
                                ...
                                <td class="py-2 px-4 border-b">
                                    <input type="text" name="quantity[]" value="{{ $item->quantity }}" required placeholder="Quantity" onkeyup="calculateTotal(event)" class="block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                </td>
                                <td class="py-2 px-4 border-b">
                                    <input type="text" name="unit_price[]" value="{{ $item->unit_price }}" required placeholder="Unit Price" onkeyup="calculateTotal(event)" class="block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                </td>
                                <td class="py-2 px-4 border-b">
                                    <input type="text" name="total_price[]" value="{{ $item->quantity * $item->unit_price }}" disabled placeholder="Total" class="cursor-not-allowed block w-full border bg-gray-100 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                    @error('total_price')
                                        <p class="text-red-500 text-md">{{ $message }}</p>
                                    @enderror
                                </td>
                    ...
                    <tfoot>
                        ...
                        <td class="py-2 px-4">
                            <input type="text" name="total_amount" value="{{ $purchase->total_amount }}" placeholder="All Total" class="cursor-not-allowed block w-full border bg-gray-100 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        </td>
                    </tfoot>
                </table>
            </div>
